...
def twinprime(n):
    """for number n, if 6 * n (+-) 1 are both prime, is n prime?"""
    if is_prime(n):
        return is_prime(6 * n - 1) and is_prime(6 * n + 1)
    else:
        return 'Non-prime number %s produces %s and %s.' % (n, (is_prime(6 * n - 1)), (is_prime(6 * n + 1)))
...
'''
prpalins = [i for i in range(2, 1003003) if i == 2 or i & 1 and ispalindrome(i) and is_prime(i)]
n = 98690 #int(input())
for i in prpalins:
    if n <= i:
        print(i)
