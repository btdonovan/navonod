...
def searcher(input):
    ...
    for c in range(len(input) - 4):
        if input[c].islower() and input[c+1:c + 4].isupper() and input[c + 4].islower() and input[c + 5:c + 8].isupper() and input[c+8].islower():
            empty.append(input[c+4])
    return ''.join(empty)

import re
def regx(input):
    return ''.join(re.findall('[^A-Z][A-Z]{3}([a-z])[A-Z]{3}[^A-Z]', input))
